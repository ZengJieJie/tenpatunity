...
#import "Masonry.h"
@interface GamenumerViewController ()
...
@implementation GamenumerViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.buttonsimageArray = [NSMutableArray array];
    self.backaray = [self generateRandomNumbers:25 min:1 max:10];
    [self createGrid];
    self.Previous=0;
    self.score=0;
    
    self.Luckynumber=[self generateRandomNumberBetween:5 and:15];
    self.lukeylabb.text=[NSString stringWithFormat:@"%d",self.Luckynumber];
