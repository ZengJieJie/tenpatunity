...
#import "Masonry.h"
@interface DoubleGmaeViewController ()
...
@implementation DoubleGmaeViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.buttonsimageArray = [NSMutableArray array];
    self.backaray = [self generateRandomNumbers:49 min:1 max:10];
    [self createGrid];
    self.Previous=0;
    self.score=0;
    
    self.Luckynumber=[self generateRandomNumberBetween:5 and:15];
    self.Luckynumber2=[self generateRandomNumberBetween:5 and:15];
    if (self.Luckynumber==self.Luckynumber2) {
        self.Luckynumber2+=2;
    }
    self.lukeylabb.text=[NSString stringWithFormat:@"%d,%d",self.Luckynumber,self.Luckynumber2];
...
- (void)buttonClicked:(UIButton *)sender {
    ...
    if (self.lastClickedButton!=nil&&self.secondLastClickedButton!=nil){
        ...
        if (acbcsum==self.Luckynumber||acbcsum==self.Luckynumber2){
            self.secondLastClickedButton.enabled=YES;
            self.secondLastClickedButton.alpha=1.0;
